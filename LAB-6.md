...
1. - count, который содержащит результаты подсчета насекомых
2. - spray, который содержащит коды инсектицидных средств (от А до F):
```
...
```
Функция `split(x, FUNCTION = ...)` - 
разбивает данные вектора или дата фрейма х в соответствии с функцией FUNCTION
```
```
